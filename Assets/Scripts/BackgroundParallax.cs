...
public class BackgroundParallax : MonoBehaviour
{
	...
	void Update ()
	{
		...
		for(int i = 0; i < backgrounds.Length; i++)
		{
            ...
            if (!String.IsNullOrEmpty(backgrounds[i].position.x.ToString()) || !String.IsNullOrEmpty(parallax.ToString()))
               {  
			backgroundTargetPosX = backgrounds[i].position.x + parallax * (i * parallaxReductionFactor + 1);
            }
